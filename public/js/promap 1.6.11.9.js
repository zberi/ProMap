...
function getConnGeometry(conn) {
  ...
  if (isBack) {
    ...
    if (isWrapDown) {
      // Exit top of source, route above ALL rows between source and target,
      // enter top of target — never dips through any row, stays clear in transit.
      const ox1 = fn.x + fw/2, oy1 = fn.y;          // top-centre of source
      const ox2 = tn.x + tw/2, oy2 = tn.y;          // top-centre of target
      // Clear above the source's own row band — use source top minus margin,
      // since source top is always the highest point either node reaches upward
      const clearY = oy1 - 50;
      segments = [
...
function drawConnection(conn, svg, geom, crossings) {
  ...
  if (isBack) {
    let d;
    if (geom.isWrapDown) {
      // Exit top of source, clear above source's row, enter top of target
      const ox1 = fn.x + fw/2, oy1 = fn.y;
