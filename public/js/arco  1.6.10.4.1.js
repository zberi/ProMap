...
function extractStepsFromReply(reply) {
  ...
  // Primary: <STEPS> tags
  const tagged = reply.match(/<STEPS>([\s\S]*?)<\/STEPS>/);
  if (tagged) {
    try {
  ...
  // Fallback: bare JSON array — find last [ that starts a steps array
  // Search from end to avoid markdown list brackets
  let searchFrom = reply.lastIndexOf('"name"');
  if (searchFrom === -1) searchFrom = 0;
  // Find the [ that precedes this
