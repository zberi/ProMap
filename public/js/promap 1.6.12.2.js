...
function onCanvasMouseUp(e) {
  if (State.dragging) {
    State.dirty = true;
    State.dragging = null;
    // LAY-03: reflow department group containers after manual drag
    // Re-render so group boxes recalculate from new node positions
    // then push any overlapping adjacent department boxes apart
...
function reflowDepartmentBands() {
  // Build dept→nodes map from current node positions
  const deptGroups = {};
  State.nodes.forEach(n => {
  ...
  // Compute bounding box per department
  const bands = Object.entries(deptGroups).map(([dept, nodes]) => {
    const ys = nodes.map(n => n.y || 0);
    const minY = Math.min(...ys) - PAD - HEADER_H;
    const maxY = Math.max(...ys) + NODE_H + PAD;
    return { dept, nodes, minY, maxY };
  }).sort((a, b) => a.minY - b.minY);

  // Push overlapping bands down sequentially
  for (let i = 1; i < bands.length; i++) {
    const prev = bands[i - 1];
    const curr = bands[i];
    const required = prev.maxY + VISUAL_GAP;
    if (curr.minY < required) {
      const shift = required - curr.minY;
      curr.nodes.forEach(n => { n.y = (n.y || 0) + shift; });
      curr.minY += shift;
      curr.maxY += shift;
    }
  }
