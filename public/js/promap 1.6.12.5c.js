...
function drawConnection(conn, svg, geom, crossings) {
  ...
  // Fix 2 — crossing elbow kink: draw small square bracket at each crossing point
  if (crossings && crossings.length) {
    const KINK = 6; // kink size in px
    crossings.forEach(pt => {
      // Direction of connector at crossing point (approx from bezier tangent)
      const ang = Math.atan2(y2 - y1, x2 - x1);
      const px = -Math.sin(ang), py = Math.cos(ang); // perpendicular
      // Small bracket: step sideways then back — visible kink
      const k = document.createElementNS('http://www.w3.org/2000/svg','path');
      k.setAttribute('d', `M${pt.x - Math.cos(ang)*KINK} ${pt.y - Math.sin(ang)*KINK}
        L${pt.x - Math.cos(ang)*KINK + px*KINK} ${pt.y - Math.sin(ang)*KINK + py*KINK}
        L${pt.x + Math.cos(ang)*KINK + px*KINK} ${pt.y + Math.sin(ang)*KINK + py*KINK}
        L${pt.x + Math.cos(ang)*KINK} ${pt.y + Math.sin(ang)*KINK}`.replace(/\n\s+/g,' '));
      k.setAttribute('stroke', strokeColor);
